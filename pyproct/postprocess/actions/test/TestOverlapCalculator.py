'''
Created on 27/02/2014

@author: victor
'''
import unittest
from pyproct.data.matrix.condensedMatrix import CondensedMatrix
import numpy
from pyproct.postprocess.actions.confSpaceComparison.overlapCalculator import OverlapCalculator


class Test(unittest.TestCase):


    def test_calculate_global_overlap(self):
        distance_matrix = CondensedMatrix([1., 0.7, 2.,
                                               0.3, 1.,
                                                    0.7])

        decomposed_clusters = [{"traj_0":[0],"traj_1":[1]},{"traj_0":[2],"traj_1":[3]}]

        self.assertEqual(0., OverlapCalculator.calculate_clustering_overlap(decomposed_clusters, distance_matrix))

        decomposed_clusters = [{"traj_0":[0],"traj_1":[1]}, {"traj_0":[2]}, {"traj_1":[3]}]

        self.assertEqual(0., OverlapCalculator.calculate_clustering_overlap(decomposed_clusters, distance_matrix))

        #TODO: needs at least one more complex example

    def test_calculate_cluster_overlap(self):
        distance_matrix = CondensedMatrix([1., 0.7,
                                               0.3])

        decomposed_cluster = {"traj_0":[0],"traj_1":[1],"traj_2":[2]}


        self.assertAlmostEqual(0.6190476352665703, OverlapCalculator.calculate_cluster_overlap(decomposed_cluster, distance_matrix),12)

        decomposed_cluster = {"traj_0":[0],"traj_1":[1]}
        self.assertAlmostEqual(1., OverlapCalculator.calculate_cluster_overlap(decomposed_cluster, distance_matrix), 12)

    def test_get_cluster_min_max_distances(self):
        # First test
        distance_matrix = CondensedMatrix([1., 0.7,
                                               0.3])

        decomposed_cluster = {"traj_0":[0],"traj_1":[1],"traj_2":[2]}

        expected_min_d = [ 0.7, 0.3, 0.3]

        min_d = OverlapCalculator.get_cluster_min_distances(decomposed_cluster, distance_matrix)

        numpy.testing.assert_array_almost_equal(min_d, expected_min_d, 8)

        #Second test
        distance_matrix = CondensedMatrix([1., 0.7, 2.,
                                               0.3, 1.,
                                                    0.7])


        decomposed_cluster = {"traj_0":[0,3],"traj_1":[1],"traj_2":[2]}

        expected_min_d = [ 0.7,  0.7,  0.3,  0.3]

        min_d =  OverlapCalculator.get_cluster_min_distances(decomposed_cluster, distance_matrix)

        numpy.testing.assert_array_almost_equal(min_d, expected_min_d, 8)

if __name__ == "__main__":
    #import sys;sys.argv = ['', 'Test.test_']
    unittest.main()
